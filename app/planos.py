...
from .models import Sensor, StatusPlano, SensorData
...
def plano_temperatura(ow_sensor):
    ...
    for index, sensor_address in enumerate(sensors_addresses):
            ...
            if temperature > 40:
                verdict = "fail"
                description = 'O sensor não passou!'
            else:
                verdict = "pass"
                description = 'O sensor passou!'

            Sensor.add_sensor(serialNumber=address, type=type, verdict=verdict, description=description)
            # sensorx = Sensor(serialNumber=address, type=type, verdict=verdict, description=description)
            # db.session.add(sensorx)
            id = Sensor.query.filter_by(id="id").first()
            SensorData.add_data(id=id, dateTime=dateTime, temperature=temperature)
            # sensor_data = SensorData(dateTime=dateTime, temperature=temperature)
...
def plano_pinos(ow_sensor):
    logger.info("Início plano de pinos")
    # Encontra status do plano e altera para complete
